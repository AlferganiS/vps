...
// const CodecDetect = require("codec-detect");
...
function getAudioContextProperties() {
  try {
    const nt_vc_context = new (window.AudioContext ||
...
function getDynamicCompressorFingerprint() {
  ...
  return new Promise((resolve, reject) => {
    try {
      let offlineAudioCtx = new (window.OfflineAudioContext ||
      ...
      if (offlineAudioCtx) {
        ...
        offlineAudioCtx.oncomplete = (evnt) => {
          sumBuffer = 0;
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (audioCtx) {
        ...
        scriptProcessor.onaudioprocess = async (bins) => {
...
function getAudioSourceHybridFingerprintAudioCtx() {
  let audioData = null;
  let hash = "";
