...
function loadWhichBrowser(cb) {
  ...
  e |=
    "WebKitCSSMatrix" in w ||
    "WebKitPoint" in w ||
    "webkitStorageInfo" in w ||
    "webkitURL" in w
      ? 16
      : 0;
  ...
  function load() {
    ...
    if (server) {
      ...
      script.src = "/scripts/whichbrowser.js";
...
function upgradeConnection(success, failure) {
  if (location.protocol == "http:") {
    ...
    httpRequest.open(
      "GET",
      "https://" + location.hostname + "/assets/upgrade",
      true
    );
    httpRequest.send();
...
function submitResults(r, c) {
  ...
  var identifier =
    typeof parameters.identifier != "undefined" ? parameters.identifier : "";
  ...
  try {
    var payload =
      ...
      "," +
      '"camouflage": "' +
      (Browsers.camouflage ? "1" : "0") +
      '",' +
      '"features": "' +
      Browsers.features.join(",") +
      '",' +
      '"browserName": "' +
      (Browsers.browser.name ? Browsers.browser.name : "") +
      '",' +
      '"browserChannel": "' +
      (Browsers.browser.channel ? Browsers.browser.channel : "") +
      '",' +
      '"browserVersion": "' +
      (Browsers.browser.version ? Browsers.browser.version.toString() : "") +
      '",' +
      '"browserVersionType": "' +
      (Browsers.browser.version ? Browsers.browser.version.type : "") +
      '",' +
      '"browserVersionMajor": "' +
      (Browsers.browser.version ? Browsers.browser.version.major : "") +
      '",' +
      '"browserVersionMinor": "' +
      (Browsers.browser.version ? Browsers.browser.version.minor : "") +
      '",' +
      '"browserVersionOriginal": "' +
      (Browsers.browser.version ? Browsers.browser.version.original : "") +
      '",' +
      '"browserMode": "' +
      (Browsers.browser.mode ? Browsers.browser.mode : "") +
      '",' +
      '"engineName": "' +
      (Browsers.engine.name ? Browsers.engine.name : "") +
      '",' +
      '"engineVersion": "' +
      (Browsers.engine.version ? Browsers.engine.version.toString() : "") +
      '",' +
      '"osName": "' +
      (Browsers.os.name ? Browsers.os.name : "") +
      '",' +
      '"osFamily": "' +
      (Browsers.os.family ? Browsers.os.family : "") +
      '",' +
      '"osVersion": "' +
      (Browsers.os.version ? Browsers.os.version.toString() : "") +
      '",' +
      '"deviceManufacturer": "' +
      (Browsers.device.manufacturer ? Browsers.device.manufacturer : "") +
      '",' +
      '"deviceModel": "' +
      (Browsers.device.model ? Browsers.device.model : "") +
      '",' +
      '"deviceSeries": "' +
      (Browsers.device.series ? Browsers.device.series : "") +
      '",' +
      '"deviceType": "' +
      (Browsers.device.type ? Browsers.device.type : "") +
      '",' +
      '"deviceIdentified": "' +
      (Browsers.device.identified ? "1" : "0") +
      '",' +
      '"deviceWidth": "' +
      screen.width +
      '",' +
      ...
      '"' +
      "}";
    console.log('new r" ', r.results);

    console.log('new p" ', payload);
...
Calculate.prototype = {
  ...
  calculate: function (prefix, data) {
    var result = true;
    var value =
      typeof data.value == "object" ? data.value : { maximum: data.value };
  ...
  getResult: function (key) {
    if (
      (match = new RegExp(key + "=(-?[0-9]+)").exec(
        this.parameters.test.results
      ))
    ) {
  ...
  toggleIndex: function () {
    if (document.body.className.indexOf(" indexVisible") == -1) {
      document.body.className =
        document.body.className.replace(" indexVisible", "") + " indexVisible";
    } else {
...
Save.prototype = {
  ...
  create: function () {
    ...
    this.popup.innerHTML +=
      ...
      encodeURIComponent(
        document.location.protocol + "//html5te.st/" + this.options.id
      ) +
      "&choe=UTF-8' width='200' height='200'>" +
      "</p>" +
      "<p>The unique id for this test is:<br><code>" +
      this.options.id +
      "</code></p>" +
      "</div>";
      ...
      if (typeof tests[i] != "string") {
        ...
        if (typeof tests[i].items != "undefined") {
          ...
          if (results[0] == results[1])
            ...
            cell.innerHTML =
              '<div><span class="partially">Partial</span> <span class="partial">○</span></div>';
        } else {
          if ((match = new RegExp(key + "=(-?[0-9]+)").exec(data.results))) {
            ...
            if (result & YES) {
              switch (true) {
                case !!(result & BUGGY):
                  cell.innerHTML =
                    '<div>Buggy <span class="buggy"></span></div>';
                  break;
                case !!(result & OLD):
                  cell.innerHTML =
                    '<div>Partial <span class="partial">○</span></div>';
                  count[1]++;
                  break;
                case !!(result & PREFIX):
                  cell.innerHTML =
                    '<div>Prefixed <span class="check">✔</span></div>';
                  count[1]++;
                  break;
                default:
                  cell.innerHTML =
                    '<div>Yes <span class="check">✔</span></div>';
                  count[1]++;
                  break;
              }
            } else {
              switch (true) {
                case !!(result & UNKNOWN):
                  cell.innerHTML =
                    '<div>Unknown <span class="partial">?</span></div>';
                  break;
                case !!(result & BLOCKED):
                  cell.innerHTML =
                    '<div>Not functional <span class="buggy">!</span></div>';
                  break;
                case !!(result & DISABLED):
                  cell.innerHTML =
                    '<div>Disabled <span class="ballot">✘</span></div>';
                  break;
                default:
                  cell.innerHTML =
                    '<div>No <span class="ballot">✘</span></div>';
                  break;
              }
            }
          } else {
            cell.innerHTML =
              '<div><span class="partially">Unknown</span> <span class="partial">?</span></div>';
          }
        }
  ...
  createSections: function (parent, tests) {
    for (var i = 0; i < tests.length; i++) {
      ...
      if (typeof tests[i].items != "undefined") {
        ...
        var status =
          typeof tests[i].status != "undefined" ? tests[i].status : "";
      ...
      if (typeof tests[i] == "string") {
        ...
        if (typeof tests[i].items != "undefined") {
          ...
          var children = this.createItems(parent, level + 1, tests[i].items, {
            id: key,
            status:
              typeof tests[i].status != "undefined"
                ? tests[i].status
                : data.status,
            urls: urls,
          });
...
FeaturePopup.prototype = {
  initialize: function (parent, data) {
    ...
    content +=
      ...
      (maximum || "✘") +
      "</em> <span>" +
      (maximum != 1 ? "Points" : "Point") +
      "</span></div>";
    content +=
      "<div class='column right'><a href='/compare/feature/" +
      data.id +
    ...
    for (var i = 0; i < data.urls.length; i++) {
      ...
      if (data.urls[i][0] == "wp")
        content +=
          "<a href='https://docs.webplatform.org/wiki" +
          data.urls[i][1] +
...
document.addEventListener(
  "click",
  function () {
    if (FeaturePopup.current) FeaturePopup.current.close();
  },
  true
);
document.addEventListener(
